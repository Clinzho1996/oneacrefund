...
function PostingDetails() {
	...
	const [userData, setUserData] = useState<Posting | null>(null);
	...
	const fetchPosting = useCallback(async () => {
		...
		try {
			...
			const response = await axios.get<ApiResponse>(
				`https://api.wowdev.com.ng/api/v1/device/posting/${id}`,
				{
					headers: {
						Accept: "application/json",
						redirect: "follow",
	...
	const handlePost = async (event: React.FormEvent) => {
		...
		try {
			...
			if (response.status === 200) {
				...
				closeModal();
				fetchPosting(); // Refresh device data
			}
		} catch (error) {
		...
		<section className="bg-[#F6F8F9] h-screen">
				...
				<div className="flex flex-row justify-start items-start gap-20 ">
						...
						<div className="py-4 w-full  border-b-[1px] border-[#E2E4E9]">
							...
							<p
								className={`status-inner mt-4 ${
									userData?.device?.status === "posted" ? "green" : "red"
								}`}>
								{userData?.device?.status}
							</p>
						</div>
						<div className="py-4 w-full  border-b-[1px] border-[#E2E4E9]">
					...
					<div className="bg-[#F6F8FA] border-[1px] border-[#E2E4E9] w-[500px] rounded-lg">
						<div className="flex flex-row justify-between items-center">
							...
							{userData?.device?.status === "posted" && (
								<Button className="bg-white mr-10" onClick={openDeleteModal}>
									Unpost
								</Button>
							)}

							{userData?.device?.status === "unposted" && (
			...
			{isDeleteModalOpen && (
				<Modal onClose={closeDeleteModal} isOpen={isDeleteModalOpen}>
					<p className="mt-4">
						Are you sure you want to unpost {userData?.device?.alias} device ?
					</p>
